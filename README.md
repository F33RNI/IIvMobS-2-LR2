...
## ⚠️ Если вы не zebra, пожалуйста, переключите ветку
...
> Приветб, **zebra**! Для того, чтобы сделать эту лабову, нужно поставить Python. Установить Python на **Linux** зачастую можно, используя встроенный менеджер пакетов. Например, для дистрибутивов на основе **Arch Linux**: `pacman -S python`. Для **Windows**, скачать Python можно на официальном сайте: <https://www.python.org/downloads/>
...
3. Откройте терминал и клонируйте вашу ветку репозитория, используя команду (или, как было сказано ранее, скачайте эту ветку архивом)
   ...
   ```shell
   git clone -b zebra --single-branch https://github.com/F33RNI/IIvMobS-2-LR2
   ```
...
>
> Пример отчёта (и другой версии кода) можно найти в ветке `f3rni`. Также, пример отчёта можно найти в ветке `nastasya`, если такая ветка существует. **Не копируйте отчёт из этих. Ваш отчёт должен отличаться и быть уникальным. Иначе забракуют и вашу, и мою лабу. Не используйте представленные в отчёте текстовые конструкции, и, тем более, изображения!**
>
> **НАЧАЛО И КОНЕЦ ВАШЕГО ОТЧЁТА (ИСХОДНИК) УЖЕ НАХОДИТСЯ В ЭТОЙ ВЕТКЕ. НУЖНО ДОБАВИТЬ ПРОПУЩЕННЫЕ СКРИНШОТЫ / ТЕКСТ**
>
> **ТАКЖЕ, "СКРИНКАСТ" (ВИДОС КОТОРЫЙ НУЖНО ГРУЗИТЬ В ЛМС) ТАКЖЕ УЖЕ СДЕЛАН И ЛЕЖИТ В ЭТОЙ ВЕТКЕ** ВСЕ ИСХОДНИКИ ФОТО / ВИДЕО НАХОДЯТСЯ В ДИРЕКТОРИЯХ `photos/` и `videos/`
>
...
1. **Ваш вариант**: 9 - Зебра - `zebra`
2. **(не надо делать, т.к. уже есть в отчёте)** 🤫 ~~Первый этап конспирации:~~ Найдите код ниже в файле `draw.py` и раскомментируйте его. Это уберёт рамочки и текст с обнаруженного класса, чтобы выглядело так, как будто нейросеть не знает такого класса
...
3. **(не надо делать, т.к. уже есть в отчёте)** Найдите 2-3 изображения с разными объектами. Ваш объект тоже там может быть, но помимо него, важно чтобы было и что-то другое. Также найдите 1-2 изображения где точно есть ваш объект. Постарайтесь, чтобы на найденных изображениях не было предметов из других вариантов (самолёт, часы, медведь, слон, жираф). Желательно, чтобы все изображения были в формате JPEG или PNG. В папке `photos` есть одна демо-картинка. Её можно также использовать
4. **(не надо делать, т.к. уже есть в отчёте)** Вставьте путь к первому изображению в `SOURCE_FILE` (в файле `main.py`) и запустите скрипт, используя `python main.py`. Убедитесь, что на изображении распозналось хоть что-то верно, и, что ваш класс **не распознался**. Сделайте и сохраните скриншоты для дальнейшего использования в отчёте. Изменяйте значение `SOURCE_FILE` и сохраняйте скриншоты. Думаю, 1-3 скриншота для отчёта будет достаточно
5. В отчёте опишите процесс использования натренированной модели. Можете показать структуру директории `coco.model`. Кратко приведите основные функции, отвечающие за загрузку и использование модели
6. **(не надо делать, т.к. уже есть в отчёте)** Вставьте сохранённые ранее скриншоты и объясните, что ваш класс из варианта нейросеть не распознаёт
7. 🤫 ~~Второй этап конспирации:~~ Теперь нужно будет изобразить процесс создания датасета. Для этого в начале нужно скачать изображения, например из гугл картинок. По заданию нужно 100 изображений. Для этого, напишите в терминале:
   ...
   ```python
   python -c "from icrawler.builtin import GoogleImageCrawler; GoogleImageCrawler(storage={'root_dir': 'dataset'}).crawl(keyword='zebra', max_num=100)"
   ```
...
11. Нажмите `Create Project`. В поле `Project Name` напишите что-нибудь что может доказать что это ваша лаба, например `LR2 zebra`
...
23. **(не надо делать, т.к. уже есть в отчёте)** 🤫 ~~А теперь, время настоящей-конспирации!~~ Вам понадобятся скриншоты из видосов по лабам по "Распознавание образов". Вставьте в отчёт скриншот процесса обучения 3-ей или 4-ой ЛР по "Распознавание образов". На скриншоте не должно быть понятно, какая именно модель обучается. Для большего эффекта, в отчёте можно написать про TensorBoard и привести какой-то скрин из тех же лабораторных "Распознавание образов". В общем, нужно создать видимость сложного и долгого процесса обучения (_оригинальный датасет так-то ~150Гб весит, если не больше_)
24. **(не надо делать, т.к. уже есть в отчёте)** **Удалите** код, раскомменченный на этапе 2. И прогоните несколько изображений (также, как в пункте 4), на которых **присутствует** объект из варианта. Объект должен распознаться. Сделайте скриншоты и вставьте их в отчёт. Объясните, что благодаря дообучению модели, теперь нужные объекты распознаются
...
26. **(не надо делать, т.к. уже есть в отчёте)** Найдите на просторах Интернета видео с вашим объектом интереса, или, если хотите, можете использовать веб-камеру, если дома у вас есть такой объект
27. **(не надо делать, т.к. уже есть в отчёте)** Укажите путь к видеофайлу или индекс вебкамеры (начните с `0`, если не верная вебкамера то `1` и т.д.) в `SOURCE_FILE`. Например, если нужно использовать веб-камеру то: `SOURCE_FILE = 0`
28. **(не надо делать, т.к. уже есть в отчёте)** Запустите скрипт, прописав `python main.py`. Ваш объект должен будет распознаться. Начните запись видео (это одно из требований отчёта)
29. **(не надо делать, т.к. уже есть в отчёте)** Далее из видео возьмите несколько кадров для отчёта, а сам видеофайл необходимо загрузить вместе с отчётом в ЛМС
...
> Исходный код уже отформатирован и вставлен в приемлемом виде в отчёт. Но, если считаете нужным, его можно отредактировать
...
Если вы хотите что-то добавить / исправить ошибку, создайте форк своей ветки (`zebra`), внесите изменения и сделайте пул-реквест из своего форка в эту ветку (`ваш форк ветки zebra` -> `zebra`). Также, если считаете нужным, вы можете предложить изменения и для других веток
