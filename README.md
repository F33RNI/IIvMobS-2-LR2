...
## ⚠️ Если вы не katiklex, пожалуйста, переключите ветку
...
3. Клонируйте вашу ветку репозитория, используя команду
   ...
   ```shell
   git clone -b katiklex --single-branch https://github.com/F33RNI/IIvMobS-2-LR2
   ```
...
2. 🤫 ~~Первый этап конспирации:~~ Найдите код ниже, раскомментируйте его и вставьте туда название класса, скопированное в 1 пункте. Это уберёт рамочки и текст с обнаруженного класса, чтобы выглядело так, как будто нейросеть не знает такого класса
   ...
   ```python
   if classes[class_index] == "clock":
       continue
   ```

3. Найдите 2-3 изображения с разными объектами. Ваш объект тоже там может быть, но помимо него, важно чтобы было и что-то другое. Также найдите 1-2 изображения где точно есть ваш объект. Постарайтесь, чтобы на найденных изображениях не было предметов из других вариантов (самолёт, книга, часы, медведь и т.д.). Желательно, чтобы все изображения были в формате JPEG или PNG
4. Вставьте путь к первому изображению в `TO_DETECT` и запустите скрипт, используя `python main.py`. Убедитесь, что на изображении распозналось хоть что-то верно, и, что ваш класс **не распознался**. Сделайте и сохраните скриншоты для дальнейшего использования в отчёте. Изменяйте значение `TO_DETECT` и сохраняйте скриншоты. Думаю, 1-3 скриншота для отчёта будет достаточно
...
7. 🤫 ~~Второй этап конспирации:~~ Теперь нужно будет изобразить процесс создания датасета. Для этого в начале нужно скачать изображения, например из гугл картинок. По заданию нужно 100 изображений. Для этого, напишите в терминале:
   ...
   ```python
   python -c "from icrawler.builtin import GoogleImageCrawler; GoogleImageCrawler(storage={'root_dir': 'dataset'}).crawl(keyword='clock', max_num=100)"
   ```
...
11. Нажмите `Create Project`. В поле `Project Name` напишите что-нибудь что может доказать что это ваша лаба, например `LR2 katiklex`
...
27. Укажите путь к видеофайлу или индекс вебкамеры (начните с `0`, если не верная вебкамера то `1` и т.д.) в `TO_DETECT`. Например, если нужно использовать веб-камеру то: `TO_DETECT = 0`
