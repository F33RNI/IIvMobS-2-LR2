...
## ⚠️ Если вы не g0n4ar0v, пожалуйста, переключите ветку
...
3. Откройте терминал и клонируйте вашу ветку репозитория, используя команду (или, как было сказано ранее, скачайте эту ветку архивом)
   ...
   ```shell
   git clone -b g0n4ar0v --single-branch https://github.com/F33RNI/IIvMobS-2-LR2
   ```
...
5. В отчёте опишите процесс использования натренированной модели. Можете показать структуру директории `model`. Кратко приведите основные функции, отвечающие за загрузку и использование модели
...
7. 🤫 ~~Второй этап конспирации:~~ Теперь нужно будет изобразить процесс создания датасета. Для этого в начале нужно скачать изображения, например из гугл картинок. По заданию нужно 100 изображений. Для этого, напишите в терминале:
   ...
   ```python
   python -c "from icrawler.builtin import GoogleImageCrawler; GoogleImageCrawler(storage={'root_dir': 'dataset'}).crawl(keyword='elephant', max_num=100)"
   ```
...
11. Нажмите `Create Project`. В поле `Project Name` напишите что-нибудь что может доказать что это ваша лаба, например `LR2 g0n4ar0v`
...
Если вы хотите что-то добавить / исправить ошибку, создайте форк своей ветки (`g0n4ar0v`), внесите изменения и сделайте пул-реквест из своего форка в эту ветку (`ваш форк ветки g0n4ar0v` -> `g0n4ar0v`). Также, если считаете нужным, вы можете предложить изменения и для других веток
...
```text
№ варианта  Объект      Название из classes.txt
