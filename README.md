...
## ⚠️ Если вы не f3rni, пожалуйста, переключите ветку
...
> Привет, **f3rni**! Для того, чтобы сделать эту лабову, нужно поставить Python. Установить Python на **Linux** зачастую можно, используя встроенный менеджер пакетов. Например, для дистрибутивов на основе **Arch Linux**: `pacman -S python`. Для **Windows**, скачать Python можно на официальном сайте: <https://www.python.org/downloads/>
...
3. Клонируйте вашу ветку репозитория, используя команду
   ...
   ```shell
   git clone -b f3rni https://github.com/F33RNI/IIvMobS-2-LR2
   ```
...
>
> Пример отчёта можно найти в ветке `f3rni`. **Не копируйте отчёт. Ваш отчёт должен отличаться и быть уникальным. Иначе забракуют и вашу, и мою лабу. Не используйте представленные в отчёте текстовые конструкции, и, тем более, изображения!**
>
...
11. Нажмите `Create Project`. В поле `Project Name` напишите что-нибудь что может доказать что это ваша лаба, например `LR2 f3rni`
