...
3. Найдите 2-3 изображения с разными объектами. Ваш объект тоже там может быть, но помимо него, важно чтобы было и что-то другое. Также найдите 1-2 изображения где точно есть ваш объект. Постарайтесь, чтобы на найденных изображениях не было предметов из других вариантов (самолёт, часы, книга, медведь, слон). Желательно, чтобы все изображения были в формате JPEG или PNG. В папке `photos` есть одна демо-картинка. Её можно также использовать
...
7. 🤫 ~~Второй этап конспирации:~~ Теперь нужно будет изобразить процесс создания датасета. Для этого в начале нужно скачать изображения, например из гугл картинок. По заданию нужно 100 изображений. Для этого, напишите в терминале:
   ...
   ```python
   python -c "from icrawler.builtin import GoogleImageCrawler; GoogleImageCrawler(storage={'root_dir': 'dataset'}).crawl(keyword='giraffe', max_num=100)"
   ```
